...
function gameOver()
{
    ...
    const gameOverMessage = `Final del juego, puntuación: - <span class = 'badge'>${score}</span> Presiona Iniciar para jugar nuevamente`
...
function chooseCard(card)
{
    if(canChooseCard())
    {
        ...
        setTimeout(() => {
            flipCards(false)
            updateStatusElement(currentGameStatusElem,"block", primaryColor,"Posición de la carta revelada")
...
function outputChoiceFeedBack(hit)
{
    if(hit)
    {
        updateStatusElement(currentGameStatusElem, "block", winColor, "Has acertado")
    }
    else
    {
        updateStatusElement(currentGameStatusElem, "block", loseColor, "Has fallado")
    }
}
...
function shuffleCards()
{
    ...
    function shuffle()
    {
        ...
        if(shuffleCount == 500)
        {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Presiona la carta que creas que es el As de picas")
